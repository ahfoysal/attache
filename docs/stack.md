...
Python for the core, TypeScript for the UI surfaces. I'm stronger in TS, so this needed justifying to myself: the voice ecosystem is Python-first — Pipecat, faster-whisper, Silero, the smart-turn model all live there, and the Node voice SDKs trail their Python versions. The Claude Agent SDK is first-class in both, so the agent layer doesn't vote. A full-TS build would move faster for two weeks and then pay for it every time a voice component only exists in Python. Two processes, one language each, over HTTP/WS — a shape I already run elsewhere.

## The picks

| Concern | Pick | The trade I'm accepting |
|---|---|---|
...
| Voice pipeline | Pipecat (BSD-2) | Most active, vendor-neutral. LiveKit Agents if I later want WebRTC room semantics |
...
| Wake word (phase 5) | Porcupine, or openWakeWord with self-trained models | openWakeWord's bundled models are CC-BY-NC — train my own or pay attention to that |
...
| Networking/auth | Tailscale everywhere; single bearer token + device list | A real IdP only if this ever leaves my tailnet. Nothing listens publicly, ever |
| Workflow engine | None — custom state machine; DBOS then Temporal are the graduation path | I own recovery correctness for ~500 lines |
...
| Subsystem | Call | Why |
|---|---|---|
| Voice pipeline | Buy (Pipecat) | Years of edge cases I don't want to rediscover |
...
| Task engine + session registry | **Build** | The product. Nothing ships my semantics |
| Memory | Build (Postgres+pgvector) | Frameworks would own my most product-defining data shape; revisit honestly at phase 3 |
| Policy engine | Build | Security semantics I must be able to hold in my head |
| Mobile app | Build (RN) | It's my daily surface; Happy (MIT) exists as reference and fallback |
