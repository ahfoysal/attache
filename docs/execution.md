...
|---|---|---|---|
| Local, trusted workspace | Agent SDK on the home base, cwd-scoped | The SDK's built-in OS sandbox (Seatbelt on macOS, bubblewrap on Linux): writes confined to the workspace, network through a domain-allowlist proxy | My own projects |
...
- **Identity**: a dedicated keypair *per enrolled machine*, generated at enrollment, held only by the gateway's secret broker. Never my personal key, never agent forwarding. The remote account is a purpose-made user with access only to declared project paths; no sudo by default.
...
- **Environment hygiene.** Executors build env from an explicit per-workspace list — the agent process never inherits my shell. Secrets are injected by reference at spawn time and pattern-redacted from all captured output.
