...
A custom state machine on Postgres — roughly 500 lines, fully understandable at 2am — rather than a workflow engine.
...
| LangGraph | Checkpointing and interrupts map well conceptually, but it wants to own the agent loop — and the agent loop here is the Agent SDK, which owns it better. Two competing runtime abstractions is worse than either alone |
...
| DBOS | The honorable mention: durable execution as an embedded library on Postgres, no extra server. If checkpointed multi-step workflows become needed, this is the graduation path before Temporal |

The trade: crash-recovery correctness becomes an owned problem. Contained, because every transition is a DB write and workers are stateless supervisors of resumable sessions.
